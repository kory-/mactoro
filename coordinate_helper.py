...
class CoordinateRecorder:
    ...
    def _update_terminal_coordinates(self):
        ...
        while self.running and self.show_coordinates_in_terminal:
            ...
            # Update cursor position on the same line
    ...
    def run_interactive_mode(self):
        ...
        print("  - Right mouse click: Record coordinates with color details")
        print("  - ESC key: End recording")
        print("  - S key: Save current recordings")
        ...
        # Start coordinate display in terminal
    ...
    def start_overlay(self):
        if TKINTER_AVAILABLE:
            # On macOS, tkinter needs to run on main thread, so overlay is disabled
            if sys.platform == 'darwin':
                print("On macOS, coordinate overlay runs in a separate process.")
                print("Current mouse coordinates will be displayed in terminal.")
            ...
            print("Warning: tkinter is not available. Coordinate overlay feature is disabled.")
            print("Coordinate recording will still work without the overlay.")
            self.show_coordinates_in_terminal = True
...
class ScreenshotAnalyzer:
    ...
    def analyze_with_gui(self, image_path):
        if not TKINTER_AVAILABLE:
            print("GUI analysis feature unavailable because tkinter is not available.")
            print("Please check coordinates manually or use Python environment with tkinter installed.")
            return
        ...
        def on_click(event):
            ...
            if 0 <= x < original_img.width and 0 <= y < original_img.height:
                ...
                self.recorded_points.append(point)
                print(f"Recorded: {point['name']} - Coordinates: ({x}, {y}) - Color: RGB{color}")
        ...
        print("\nControls:")
        print("  - Mouse hover: Display coordinates and color")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Coordinate verification helper tool")
    ...
    if args.mode == 'record':
        ...
        if filename and not args.image:
            print(f"\nAnalyze the screenshot? (y/n): ", end='')
            if input().lower() == 'y':
                analyzer.analyze_with_gui(filename)
    
    elif args.mode == 'analyze':
        if not args.image:
            print("Error: Please specify an image file with --image option")
            sys.exit(1)
